...
if i=length tr then
  if \<exists>t ts. get_action a = ABeginAtomic t ts then
       sessionsInTransaction tr (length tr - 1) \<union> {get_invoc a}
  else if get_action a = AEndAtomic then
       sessionsInTransaction tr (length tr - 1) - {get_invoc a}
  else 
...
proof (intro if_cases2; clarsimp?)
  ...
  assume a0: "i = length tr"
    and a1: "get_action a = ABeginAtomic t ts"

  then show "sessionsInTransaction (tr @ [a]) (length tr) = insert (get_invoc a) (sessionsInTransaction tr (length tr - Suc 0))"
  proof (cases a, auto)
...
next
  assume a0: "i = length tr"
    and a1: "get_action a = AEndAtomic"

  then show "sessionsInTransaction (tr @ [a]) (length tr) = sessionsInTransaction tr (length tr - Suc 0) - {get_invoc a}"
    apply (cases a, auto)
      apply (auto simp add: nth_append sessionsInTransaction_def inTransaction_def split: if_splits)[1]
...
next
  assume a0: "i = length tr"
    and a1: "\<forall>t ts. get_action a \<noteq> ABeginAtomic t ts"
    and a2: "get_action a \<noteq> AEndAtomic"
...
next
  ...
  next
    ...
    have [simp]: "currentTransaction S' (get_invoc x) = Some tx"
      using local.Cons step.IH step.prems by force

    have "get_invoc (last tr) = invoc"
...

    {
      ...
      have a_allowed_ctxt_switch: "get_invoc a = invoc"
        by (metis \<open>get_invoc (last tr) = invoc\<close> diff_less j_def last_conv_nth length_greater_0_conv less_numeral_extra(1) list.simps(3) local.Cons nth_append_first nth_append_length nth_mem step.prems(6) step.prems(7)) 
...
proof -
  ...
  have "get_invoc ( drop (Suc i) (take k tr) ! (j - Suc i)) = invoc"
  proof (rule packedTraces_stay_in_transaction2)
    ...
    show  "drop (Suc i) (take k tr) \<noteq> [] \<Longrightarrow> get_invoc (hd (drop (Suc i) (take k tr))) = invoc"
  ...
  qed
  then show "get_invoc (tr ! j) = invoc "
